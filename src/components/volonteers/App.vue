<template>
  <div class="row">
    ...
    <div class="col-xs-12 col-sm-12 col-md-10 col-lg-10 col-xl-10">
      <breadcrumbs-component type="volonteers" v-if="mode === 'bitrix'"/>
      <table-component
  ...
  methods: {
    ...
    async getData() {
      try {
        ...
        this.$store.dispatch('showNotification', {
          text:
            'Ошибка загрузки справочника волонтеров! ' +
    ...
    async handleDelete(id) {
      try {
        ...
        await this.getData();
        this.$store.dispatch('showNotification', {
          text: res.text ? res.text : 'Волонтер успешно удален!',
          type: 'success',
        });
      } catch (e) {
        this.$store.dispatch('showNotification', {
          text:
            'Не удалось удалить волонтера! ' +
            (e && e.message ? e.message : ''),
          type: 'error',
        });
      }
