<template>
  <div class="row">
    ...
    <div class="col-xs-12 col-sm-12 col-md-10 col-lg-10 col-xl-10">
      <breadcrumbs-component type="timenorms" v-if="mode === 'bitrix'"/>
      <table-component
...
export default {
  ...
  methods: {
    ...
    async getData() {
      try {
        const { data } = await axios(this.urls.listItems.replace('{name}', this.$route.name));
        ...
        this.$store.dispatch('showNotification', {
          text:
            'Ошибка загрузки справочника норм времени! ' +
    ...
    async handleDelete(id) {
      try {
        ...
        await this.getData();
        this.$store.dispatch('showNotification', {
          text: res.text ? res.text : 'Норма времени успешно удалена!',
          type: 'success',
        });
      } catch (e) {
        this.$store.dispatch('showNotification', {
          text:
            'Не удалось удалить норму времени! ' +
            (e && e.message ? e.message : ''),
          type: 'error',
        });
      }
