<template>
  <div class="row">
    ...
    <div class="col-xs-12 col-sm-12 col-md-10 col-lg-10 col-xl-10">
      <breadcrumbs-component type="phonebook" v-if="mode === 'bitrix'"/>
      <table-component
...
export default {
  ...
  methods: {
    ...
    async getData() {
      try {
        const { data } = await axios.get("/phonebook/index");
    ...
    async handleDelete(id) {
      try {
        ...
        this.$store.dispatch("showNotification", {
          text: res.text ? res.text : "Номер телефона успешно удален!",
          type: "success"
        });
        await this.getData();
      } catch (e) {
        this.$store.dispatch("showNotification", {
          text:
            "Не удалось удалить номер телефона! " + (e && e.message ? e.message : ''),
          type: "error"
        });
      }
    },
