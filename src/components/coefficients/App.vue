<template>
  <div class="row">
    ...
    <div class="col-xs-12 col-sm-12 col-md-10 col-lg-10 col-xl-10">
      <breadcrumbs-component type="coefficients"/>
      <table-component
...
export default {
  ...
  methods: {
    ...
    async getData() {
      try {
        const { data } = await axios.get("/coefficient/index");
        ...
        this.$store.dispatch("showNotification", {
          text:
            "Ошибка загрузки справочника коэффициентов! " +
    ...
    async handleDelete(id) {
      try {
        ...
        this.$store.dispatch("showNotification", {
          text: res.text ? res.text : "Коэффициент успешно удален!",
          type: "success"
        });
        await this.getData();
      } catch (e) {
        this.$store.dispatch("showNotification", {
          text:
            "Не удалось удалить коэффициент! " +
            (e && e.message ? e.message : ""),
          type: "error"
        });
      }
